...
namespace UclaExt.Common.ExtensionMethods
{
    public static class AdoExtensions
    {
        ...
        #region Alternative Mapping Extensions
        // THESE ARE EXPERIMENTAL, IT'S NOT CLEAR THAT JUST 
        // CASTING TO A SPECIFIC TYPE IS ADVISABLE


        /// <summary>
        /// Map a column in a data reader result set to a .Net type T 
        /// based on the column position.
        /// </summary>
        /// <typeparam name="T">.Net type/></typeparam>
        /// <param name="reader">An IDataReader positioned on a row</param>
        /// <param name="ordinal">The ordinal position of the column in the result set - first column = 0.</param>
        /// <returns>Mapped value</returns>
        public static T MapTo<T>(this IDataReader reader, int ordinal)
        {
            T val = default(T);
            try
            {
                ...
                {
                    return val; // Nullable<T> types return null
                }
                else if (val is ValueType)
                {
                    throw new InvalidCastException("Attempt to convert data reader column " + ordinal
                    + " to " + typeof(T).Name);
                }
                else
                {
                    return val; // reference types return null
                }
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException("Failed to convert data reader column "
                    + ordinal + " value to type " + typeof(T).GetType(), ex);
            }
        }
        ...
        /// <returns>Mapped value</returns>
        public static T MapTo<T>(this IDataReader reader, string columnName)
        {
            int ordinal = 0;
            try
            {
            ...
            {
                throw new IndexOutOfRangeException("Column Name " + columnName
                    + " does not match a column in the result set.", ex);
            }

            return reader.MapTo<T>(ordinal);
        }
